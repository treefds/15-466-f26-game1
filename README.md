...
The game has a total of 3 levels. The palette is restricted to NES colors.
...
1. Build `Assets.hpp` from the spritesheet (`sprites/spritesheet.png`) and palette index sheet  (`sprites/colorsheet.png`):
   ...
   5. Everything is written into `Assets.hpp`. `PlayMode` would then load constants from the file into the PPU.
...
DESIGN NOTES (hopefully, **not needed** for clearing the game)
...
- I expect the game to be beatable in about 5 minutes?
